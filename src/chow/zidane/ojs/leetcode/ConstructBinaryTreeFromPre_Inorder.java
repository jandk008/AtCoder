...
public class ConstructBinaryTreeFromPre_Inorder {
	...
	public TreeNode buildTree(int[] preorder, int[] inorder) {
		...
		while (!list.isEmpty()) {
			...
			boolean hasLeft = leftChildSize > 0 ? true : false;
			if (hasLeft) {
				l = i + 1;
			...
			boolean hasRight = rightChildSize > 0 ? true : false;
			if (hasRight) {
				r = i + leftChildSize + 1;
	...
	static class Recursion2 {
		...
		private TreeNode buildTree(int[] preorder, int idx, int[] inorder,
				int end, int start) {
	...
	public static void main(String[] args) {
		new ConstructBinaryTreeFromPre_Inorder().buildTree(new int[] { 1, 2, 4,
